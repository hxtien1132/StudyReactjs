...
const PhotoItem =({info: {url,isFavorite,id}})=>{
  ...
  const { tonggleFavorite,addToCart} = useGallery();
    ...
    return (
        ...
        >
          ...
          />
            ...
            <button className="py-3 px-6 bg-white text-sm font-medium rounded-lg text-black absolute bottom-5 left-2/4 -translate-x-2/4
            opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all" onClick={()=> addToCart(items)}
            >Add to card</button>
        </div>
    )
