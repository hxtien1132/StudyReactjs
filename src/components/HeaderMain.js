...
import { useGallery } from "./b10-gallery/gallery-context";
import { useAuth } from "./Ex-hook/useContext/AuthContext";

const HeaderMain = () => {
  const { user, setUser} = useAuth();
  const {photos,cartItems } = useGallery()
  const favoriteCount = photos.filter(item => item.isFavorite === true)?.length  ;
// c2:   const favoriteCount = photos.length > 0 ?  photos.filter(item => item.isFavorite === true).length : 0;
  const cardItemsCount = cartItems.length
  // console.log(user);
  return (
      ...
      <div className="ml-auto mr-5 flex items-center gap-5">
        <span className="relative w-8">
          ...
          <span className="w-5 h-5 rounded-full flex items-center justify-center text-white
           bg-pink-500 absolute right-0 top-0 translate-x-2/4">
         { cardItemsCount ||  0}
          </span>
        </span>
        <span className="relative w-8">
