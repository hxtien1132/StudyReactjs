...
const MovieSearchMovie = () => {
    const [movies,setMovies] =useState([]) // api trả về
    const [query,setQuery] = useState("")//search giá trị
    const queryDebounce  = useDebounce(query,500) //delay api
    // console.log(queryDebounce);
    const [loading,setLoading] = useState(true)//làm icon load
  useEffect(() => {
    async function fetchData() {
      setLoading(true)
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=5a1549eca98425d82d3d4bedb789e02b&query='${queryDebounce}'`
      );
      ...
      <div className="w-full max-w-[500px] mx-auto mb-20">
        <input
          ...
          className="w-full p-5 rounded-lg border border-purple-500 
                shadow-[0px_0px_0px_3px_rbga(125,_126,_255,_0.2)]"
...
export default MovieSearchMovie;
